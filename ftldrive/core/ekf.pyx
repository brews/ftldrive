...
def sequential_ekf(state, obs_value, obs_error, obs_idx):
    ...
    state = fast_obs_assimilation(state, obs, error, idx)
...
cdef double[:,:] fast_obs_assimilation(double[:,:] state, double[:] obs_value, double[:] obs_error, long[:] obs_idx):
    ...
    for i in range(n):
        ...
        state[...] = sequential_filter(prior_ensemble=state,
                                       obs=obs_value[i],
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef double[:,:] sequential_filter(double[:,:] prior_ensemble, double obs, double[:] obs_estimate, double obs_error):
    ...
    cdef double[:,:] prior_anomaly = np.subtract(prior_ensemble, prior_mean[:, None])  # "None" means replicate in this dimension
    ...
    cdef double[:] gain_numer = prior_anomaly @ np.transpose(obs_estimate_anomaly) / (n_ensemble - 1)
